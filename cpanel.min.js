...
function finishAdminSplash(type="success",message="Listo"){
  ...
  const wait=Math.max(520,760-elapsed);
  clearTimeout(adminSplashCloseTimer);
...
function renderAll(){
  ...
  fillCategorySelects();renderProducts();renderCategories();renderBanners();renderOrders();renderRequests();renderVirtual();renderQuotes();renderWebContent();renderUsers();renderSettings();applyCurrentUser();
}
...
window.changeStatus=async(kind,id,status)=>{await runAdminAction(null,async()=>{await AleAPI.post("updateStatus",{kind,id,status},token);await reload();return true},{loading:"Actualizando estado...",success:"Estado actualizado correctamente",error:"No fue posible actualizar el estado"})};
...
window.toggleAdminUser=async(id,active,btn)=>{await runAdminAction(btn,async()=>{await AleAPI.post("setAdminUserActive",{id,activo:!!active},token);await reload()},{loading:active?"Activando...":"Desactivando...",success:active?"Usuario activado":"Usuario desactivado",error:"No fue posible actualizar el usuario"})};
...
$("#saveAdminUser")?.addEventListener("click",async e=>{
  ...
  await runAdminAction(btn,async()=>{
    ...
    await AleAPI.post("saveAdminUser",payload,token);$("#adminUserEditor").classList.add("hidden");await reload();
...
function renderRealtimeAdminSections(){
  ...
  try{renderOrders();renderRequests();renderVirtual();renderQuotes()}catch(e){console.warn('Render realtime parcial',e)}
}
